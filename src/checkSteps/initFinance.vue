  ...
  <div class="init-finance">
    ...
    <el-table :data="tableData" border class="mt10px init-table" style="width: 100%">
      ...
      <el-table-column prop label="银行订单时间" width="95">
        <span slot-scope="scopeOut" v-html="$timeFormat(scopeOut.row.tm,true)"></span>
      </el-table-column>
      <el-table-column prop="no" label="交易订单号" width="160"></el-table-column>
      <el-table-column prop="fm_name" label="付款名" width="120"></el-table-column>
      <el-table-column prop="fm_account" label="付款账号" width="100"></el-table-column>
      <el-table-column prop="fm_uid" label="现金收款人" width="120"></el-table-column>
      <el-table-column prop="company_name" label="付款公司名" width="120"></el-table-column>
      <el-table-column prop="remark" label="摘要|备注" width="120"></el-table-column>
      <el-table-column prop label="交易金额" width="100">
        <span slot-scope="scopeOut">{{scopeOut.row.amount | currency}}</span>
      </el-table-column>
      <!-- 拆 -->
      <el-table-column prop label="拆分后金额" class-name="splited-col" width="110">
        <template slot-scope="scopeOut">
          <el-table
            class="split-item"
            :data="scopeOut.row.split"
            :show-header="false"
            :row-class-name="scopeOut.row.split.length>1?'add-border':''"
          >
        ...
        <template slot-scope="scopeOut">
          ...
          >
            <el-table-column class-name="split-item-col" show-overflow-tooltip label prop="''">
              <span
                slot-scope="scope"
                :class="curid===scope.row.curid?'red':''"
      ...
      <el-table-column prop label="分配时间" class-name="splited-col" width="155">
        <template slot-scope="scopeOut">
          <el-table
            class="split-item"
            :data="scopeOut.row.split"
    ...
    <el-card v-if="pid==='BAITUI' || pid==='ZTC2'" class="mt10px card-total">
      <div class="money-total">
        ...
        <div>
          <b>实际到账金额：</b>
          {{orderInfo.amount_real | currency1}}
        </div>
      </div>
    </el-card>
          ...
          <el-col :md="4">
            <el-form-item :label="index?'':' 收款方式'">
              <el-select v-model="p.receivetype" placeholder="选择到款方式">
                <el-option
                  v-for="(ty,index) in orderFlowDatas"
                  :key="index"
                  :label="ty.code_desc"
                  :value="ty.code_val+'#'+ty.bsid"
                ></el-option>
              </el-select>
            </el-form-item>
          </el-col>
          <el-col :md="4">
            <el-form-item :label="index?'':' 时间'">
              <el-select v-model="p.receivetime" placeholder="选择时间">
                <el-option
                  v-for="(tm,index) in orderFlowDatas"
                  :key="index"
                  :label="tm.tm | timeFormat1"
                  :value="tm.tm"
                ></el-option>
              </el-select>
            </el-form-item>
          ...
          <el-col :md="4">
            <el-form-item :label="index?'':'增/删'">
              <el-button
                ...
                :type="index?'danger':'success'"
                :icon="index?minusIcon:addIcon"
                size="mini"
              ></el-button>
            </el-form-item>
          </el-col>
    ...
    <el-row>
      <b>总到账金额：</b>
      <span>{{totalReceive | currency1}}</span>
    </el-row>
    ...
    <div class="mt10px">
      <el-form :inline="true">
        ...
        <el-form-item label="提单时间：">
          <el-date-picker v-model="billTime" type="date" value-format="yyyy-MM-dd" placeholder="选择日期"></el-date-picker>
        </el-form-item>
        <el-form-item>
