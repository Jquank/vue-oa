  ...
  <div class="view-detail child-component-container media-padding">
    <div class="detail-content">
      <el-tabs type="border-card"
               @tab-click="tab">
        <!-- 订单基本信息 -->
        <el-tab-pane v-if="pid==='BAITUI'"
                     label="百度订单基本信息">
          <el-row class="text-right mb10px"
                  v-if="permissions.indexOf('6z')>-1">
            <el-button type="warning"
                       @click.native="changeInfo"
                       class="xsbtn">修改基本信息</el-button>
          </el-row>
          <el-table :data="baituiBasicInfo"
                    border
                    class="table-width">
            <el-table-column prop="type"
                             label="信息分类"
                             :width="tableFirstColumWidth">
            </el-table-column>
              ...
              <template slot-scope="scope">
                <div v-if="scope.$index===0">
                  <div class="row-container">
                    ...
                    <div>
                      <b>PC网址：</b>
                      <a :href="((orderInfo.pcwebsite+'').indexOf('http')>-1?'':'http://')+orderInfo.pcwebsite"
                         target="_blank">{{orderInfo.pcwebsite}}</a>
                    </div>
                    <div>
                      <b>手机网址：</b>
                      <a :href="((orderInfo.mobilewebsite+'').indexOf('http')>-1?'':'http://')+orderInfo.mobilewebsite"
                         target="_blank">{{orderInfo.mobilewebsite}}</a>
                    </div>
                  </div>
                  <div v-for="(c,index) in contactInfoList"
                       :key="index"
                ...
                <div v-if="scope.$index===2">
                  ...
                  <!-- 老合同 -->
                  <div v-if="orderInfo.ordernumber && orderInfo.ordernumber.indexOf('BJHT')==-1"
                       class="mt10px row-container">
                    <div>
                      <b>百度推广服务订单编号：</b>
        ...
        <el-tab-pane v-if="pid==='ZTC2' || pid==='ZTC'"
                     label="直通车订单基本信息">
          <el-table :data="ztcBasicInfo"
                    border
                    class="table-width">
            <el-table-column prop="type"
                             label="信息分类"
        ...
        <el-tab-pane v-if="pid==='WEBSITE' || pid==='ZTC_WEBSITE'"
                     label="网建订单基本信息">
          <el-table :data="wjBasicInfo"
                    border
                    class="table-width">
            <el-table-column prop="type"
                             label="信息分类"
        ...
        <el-tab-pane v-if="pid_ka==='KA'"
                     label="大客订单基本信息">
          <el-table :data="kaBasicInfo"
                    border
                    class="table-width">
            <el-table-column prop="type"
                             label="信息分类"
        ...
        <el-tab-pane label="发票信息">
          <el-table :data="invoiceBasicInfo"
                    border
                    class="table-width">
            <el-table-column prop="type"
                             label="信息分类"
            ...
            <el-table-column>
              <template slot="header" slot-scope="scope">
                <div class="invoice-header">
                  <span>详细信息</span>
                  <el-button v-if="permissions.indexOf('55') >-1&&invoiceInfo.cpmpanyname" @click.native="editInvoiceDialog = true" type="warning" size="mini">编辑发票</el-button>
                </div>
              </template>
              <template slot-scope="scope">
        ...
        <el-tab-pane v-if="pid_ka!=='KA'"
          ...
          <div v-if="permissions.indexOf('5c') > -1" class="qualify">
            <show-qualify v-if="showQualify.length" :showDel="false"
            :showQualify="showQualify" class="qualify-left"></show-qualify>
            <div class="qualify-right">
              <el-button @click.native="editQualify"
                         type="warning"
                         class="xsbtn">修改资质</el-button>
            </div>
          </div>
        </el-tab-pane>
        <!-- 信息 -->
        <el-tab-pane v-if="pid==='BAITUI'"
                     label="百度推广信息">
          <el-table :data="baituiExtendInfo"
                    border
                    class="table-width">
            <el-table-column prop="type"
                             label="信息分类"
            ...
            <el-table-column prop="data"
              ...
              <div slot-scope="scope">
                <div v-if="scope.$index===0">
                  ...
                  <div>
                    <div v-for="(item,index) in moneyInfo"
                         :key="index">
            ...
            <el-table-column prop="data"
              ...
              <div slot-scope="scope">
                <div v-if="scope.$index===0">
                  ...
                  <div class="mt10px row-container">
                    <div v-for="(item,index) in moneyInfo9"
                         :key="index">
                  ...
                  <div class="row-container">
                    ...
                    <div>
                      <div v-if="productInfo[0].websitetype != 30">
                        <b>网站栏目：</b>
                        <div v-for="(item,index) in JSON.parse(productInfo[0].column)"
                             :key="index">
        ...
        <el-tab-pane label="审核记录">
          <el-steps :active="isChecked"
                    space="14%"
                    finish-status="success">
            <el-step v-for="step in stepList.slice(0,8)"
                     :key="step.id"
                     :title="step.name"
                     style="margin-top:15px">
              <span v-if="step.status>0"
                    slot="description">{{step.opt_time | timeFormat}}</span>
            </el-step>
          </el-steps>
          <el-steps :active="isNextChecked"
                    space="12.5%"
                    finish-status="success">
            <el-step v-for="(step,index) in stepList.slice(8)"
                     :key="step.id"
                     :title="step.name"
                     style="margin-top:15px">
        ...
        <!-- 订单处理 -->
        <el-tab-pane v-if="mark!=='from_orderList'"
                     label="订单处理">
          <!-- 理单员审核(质检经理) -->
          <order-keeper v-if="sn===20 && templateInfo.cpid"
                        :moneyInfo="moneyInfo"
                        ...
                        :pid="pid"
                        title="理单员审核"></order-keeper>
          <!-- 综合部初审 -->
          <init-finance v-if="sn===100 && moneyInfo.length"
                        :moneyInfo="moneyInfo"
                        :moneyRecord="moneyRecord"
                        :orderFlowDatas="orderFlowDatas"
                        :orderInfo="orderInfo"
                        ...
                        :invoiceInfo="invoiceInfo"
                        :pid="pid"
                        title="综合部初审"></init-finance>
          <!-- 业支外审 -->
          <out-quality v-if="sn===120 && templateInfo.cpid"
                       :moneyInfo="moneyInfo"
                       :moneyRecord="moneyRecord"
                       :orderFlowDatas="orderFlowDatas"
                       :orderInfo="orderInfo"
                       ...
                       :invoiceInfo="invoiceInfo"
                       :pid="pid"
                       title="业支外审"></out-quality>
          <!-- 业支开户 -->
          <business-support-account v-if="sn===150 && templateInfo.cpid"
                                    :moneyInfo="moneyInfo"
                                    ...
                                    :invoiceInfo="invoiceInfo"
                                    :pid="pid"
                                    title="业支开户"></business-support-account>
          <!-- 业支派单 -->
          <in-quality-order v-if="sn===160 && templateInfo.cpid"
                            :moneyInfo="moneyInfo"
                            :moneyRecord="moneyRecord"
                            :orderFlowDatas="orderFlowDatas"
                            :orderInfo="orderInfo"
                            ...
                            :invoiceInfo="invoiceInfo"
                            :pid="pid"
                            title="直通车接口派单"></in-quality-order>
          <!-- 客服申请加款（直通车） -->
          <quality-add-money v-if="sn===170 && templateInfo.cpid"
                             :moneyInfo="moneyInfo"
                             :moneyInfo9="moneyInfo9"
                             :moneyRecord="moneyRecord"
                             :orderFlowDatas="orderFlowDatas"
                             ...
                             :invoiceInfo="invoiceInfo"
                             :pid="pid"
                             title="客服申请加款"></quality-add-money>
          <!-- 业支内审（直通车） -->
          <out-quality v-if="sn===190 && templateInfo.cpid"
                       :moneyInfo="moneyInfo"
                       :moneyRecord="moneyRecord"
                       :orderFlowDatas="orderFlowDatas"
                       :orderInfo="orderInfo"
                       ...
                       :invoiceInfo="invoiceInfo"
                       :pid="pid"
                       title="直通车业支内审"></out-quality>
          <!-- 业支外审（直通车） -->
          <out-quality v-if="sn===200 && templateInfo.cpid && pid==='ZTC_WEBSITE'"
                       :moneyInfo="moneyInfo"
                       :moneyRecord="moneyRecord"
                       :orderFlowDatas="orderFlowDatas"
                       :orderInfo="orderInfo"
                       ...
                       title="业支外审"></out-quality>
          <!-- 业支主管审核（直通车） -->
          <out-quality v-if="sn===200 && templateInfo.cpid && pid==='ZTC2'"
                       :moneyInfo="moneyInfo"
                       :moneyRecord="moneyRecord"
                       :orderFlowDatas="orderFlowDatas"
                       :orderInfo="orderInfo"
                       ...
                       :invoiceInfo="invoiceInfo"
                       :pid="pid"
                       title="业支主管审核"></out-quality>
          <!-- 质检外审 -->
          <out-quality v-if="sn===200 && templateInfo.cpid && pid==='BAITUI'"
                       :moneyInfo="moneyInfo"
                       :moneyRecord="moneyRecord"
                       :orderFlowDatas="orderFlowDatas"
                       :orderInfo="orderInfo"
                       ...
                       :invoiceInfo="invoiceInfo"
                       :pid="pid"
                       title="质检外审"></out-quality>
          <!-- 质检内审派单 -->
          <in-quality-order v-if="sn===210 && templateInfo.cpid"
                            :moneyInfo="moneyInfo"
                            :moneyRecord="moneyRecord"
                            :orderFlowDatas="orderFlowDatas"
                            :orderInfo="orderInfo"
                            ...
                            :invoiceInfo="invoiceInfo"
                            :pid="pid"
                            title="质检内审派单"></in-quality-order>
          <!-- 网数派单（直通车网建） -->
          <in-quality v-if="sn===220 && templateInfo.cpid && pid==='ZTC_WEBSITE'"
                      :moneyInfo="moneyInfo"
                      :moneyRecord="moneyRecord"
                      :orderFlowDatas="orderFlowDatas"
                      :orderInfo="orderInfo"
          ...
          <in-quality v-if="sn===220 && templateInfo.cpid && pid!=='ZTC_WEBSITE'"
                      ...
                      :invoiceInfo="invoiceInfo"
                      :pid="pid"
                      title="质检内审"></in-quality>
          <!-- 网建外审 -->
          <in-quality v-if="sn===230 && templateInfo.cpid"
                      :moneyInfo="moneyInfo"
                      :moneyRecord="moneyRecord"
                      :orderFlowDatas="orderFlowDatas"
                      :orderInfo="orderInfo"
                      ...
                      :invoiceInfo="invoiceInfo"
                      :pid="pid"
                      title="网建外审"></in-quality>
          <!-- 网开空域 -->
          <web-space v-if="sn===240 && templateInfo.cpid"
                     :moneyInfo="moneyInfo"
                     :moneyRecord="moneyRecord"
                     :orderFlowDatas="orderFlowDatas"
                     :orderInfo="orderInfo"
                     ...
                     :invoiceInfo="invoiceInfo"
                     :pid="pid"
                     title="网开空域"></web-space>
          <!-- 质检申请加款 -->
          <quality-add-money v-if="sn===260 && templateInfo.cpid && pid === 'BAITUI'"
                             :moneyInfo="moneyInfo"
                             :moneyInfo9="moneyInfo9"
                             :moneyRecord="moneyRecord"
                             :orderFlowDatas="orderFlowDatas"
                             ...
                             :invoiceInfo="invoiceInfo"
                             :pid="pid"
                             title="质检申请加款"></quality-add-money>
          <!-- 理单员申请加款 -->
          <quality-add-money v-if="sn===260 && templateInfo.cpid && pid_ka === 'KA'"
                             :moneyInfo="moneyInfo"
                             :moneyInfo9="moneyInfo9"
                             :moneyRecord="moneyRecord"
                             :orderFlowDatas="orderFlowDatas"
                             ...
                             :invoiceInfo="invoiceInfo"
                             :pid="pid"
                             title="理单员申请加款"></quality-add-money>
          <!-- 网建经理派单 -->
          <web-order v-if="sn===260 && templateInfo.cpid && (pid==='WEBSITE'||pid==='ZTC_WEBSITE')"
                     :moneyInfo="moneyInfo"
                     :moneyRecord="moneyRecord"
                     :orderFlowDatas="orderFlowDatas"
                     :orderInfo="orderInfo"
                     ...
                     :invoiceInfo="invoiceInfo"
                     :pid="pid"
                     title="网建经理派单"></web-order>
          <!-- 网站制作 -->
          <web-make v-if="sn===280 && templateInfo.cpid && (pid==='WEBSITE'||pid==='ZTC_WEBSITE')"
                    :moneyInfo="moneyInfo"
                    :moneyRecord="moneyRecord"
                    :orderFlowDatas="orderFlowDatas"
                    :orderInfo="orderInfo"
                    ...
                    title="网站制作"></web-make>
          <!-- 网建内审 -->
          <web-make v-if="sn===300 && templateInfo.cpid && (pid==='WEBSITE'||pid==='ZTC_WEBSITE')"
                    :moneyInfo="moneyInfo"
                    :moneyRecord="moneyRecord"
                    :orderFlowDatas="orderFlowDatas"
                    :orderInfo="orderInfo"
                    ...
                    :invoiceInfo="invoiceInfo"
                    :pid="pid"
                    title="网建内审、网站提出"></web-make>
          <!-- 质检经理审核 -->
          <quality-manager v-if="sn===300 && templateInfo.cpid && pid !== 'WEBSITE' && pid !=='ZTC_WEBSITE'"
                           :moneyInfo="moneyInfo"
                           :moneyRecord="moneyRecord"
                           :orderFlowDatas="orderFlowDatas"
                           :orderInfo="orderInfo"
                           ...
                           :invoiceInfo="invoiceInfo"
                           :pid="pid"
                           title="质检经理审核"></quality-manager>
          <!-- 综合部复审 -->
          <second-finance v-if="sn===305 && templateInfo.cpid"
                          :moneyInfo="moneyInfo"
                          :moneyRecord="moneyRecord"
                          :orderFlowDatas="orderFlowDatas"
                          :orderInfo="orderInfo"
                          ...
                          :invoiceInfo="invoiceInfo"
                          :pid="pid"
                          title="综合部复审"></second-finance>
          <!-- 账户加款(已移动到续费加款列表，暂时没用) -->
          <finance-add-money v-if="sn===310 && templateInfo.cpid"
                             :moneyInfo="moneyInfo"
                             :moneyRecord="moneyRecord"
                             :orderFlowDatas="orderFlowDatas"
                             :orderInfo="orderInfo"
                             ...
                             :invoiceInfo="invoiceInfo"
                             :pid="pid"
                             title="账户加款"></finance-add-money>
          <!-- 质检开户(加款完成) -->
          <quality-open-account v-if="sn===320 && templateInfo.cpid"
                                :moneyInfo="moneyInfo"
                                :moneyRecord="moneyRecord"
                                :orderFlowDatas="orderFlowDatas"
                                :orderInfo="orderInfo"
                                ...
                                :invoiceInfo="invoiceInfo"
                                :pid="pid"
                                title="质检开户(加款完成)"></quality-open-account>
          <!-- 质检一部 -->
          <quality-one v-if="sn===330 && templateInfo.cpid"
                       :moneyInfo="moneyInfo"
                       :moneyRecord="moneyRecord"
                       :orderFlowDatas="orderFlowDatas"
                       :orderInfo="orderInfo"
                       ...
                       :invoiceInfo="invoiceInfo"
                       :pid="pid"
                       title="质检一部"></quality-one>
          <!-- 客服提单 -->
          <service-order v-if="sn===400 && templateInfo.cpid"
                         :moneyInfo="moneyInfo"
                         :moneyRecord="moneyRecord"
                         :orderFlowDatas="orderFlowDatas"
                         :orderInfo="orderInfo"
                         ...
                         :invoiceInfo="invoiceInfo"
                         :pid="pid"
                         title="客服提单"></service-order>
          <!-- 客服派单 -->
          <service-dispatch v-if="sn===410 && templateInfo.cpid"
                            :moneyInfo="moneyInfo"
                            :moneyRecord="moneyRecord"
                            :orderFlowDatas="orderFlowDatas"
                            :orderInfo="orderInfo"
                            ...
                            :invoiceInfo="invoiceInfo"
                            :pid="pid"
                            title="客服派单"></service-dispatch>
          <!-- 提单完成 -->
          <order-done v-if="sn===420 && templateInfo.cpid"
                      :moneyInfo="moneyInfo"
                      :moneyRecord="moneyRecord"
                      :orderFlowDatas="orderFlowDatas"
                      :orderInfo="orderInfo"
                      ...
                      :invoiceInfo="invoiceInfo"
                      :pid="pid"
                      title="提单完成"></order-done>
          <!-- 新客内审 -->
          <new-cus-check v-if="sn===430 && templateInfo.cpid"
                         :moneyInfo="moneyInfo"
                         :moneyRecord="moneyRecord"
                         :orderFlowDatas="orderFlowDatas"
                         :orderInfo="orderInfo"
        ...
        <el-tab-pane :label="logLabel">
          <el-card class="box-card">
            <div slot="header"
                 class="clearfix">
              <el-button type="text"
                         size="medium">填写日志：</el-button>
              <el-input v-model="logMark"
                        type="textarea"
                        :rows="3"></el-input>
              <el-button @click.native="subLog"
                         class="mt10px"
            ...
            <page class="page"
                  :url="logUrl"
                  :sendParams="logParams"
                  @updateList="updateLogList"></page>
          </el-card>
        </el-tab-pane>
        <!-- 记业绩 -->
        <el-tab-pane label="记业绩"
                     v-if="permissions.indexOf('4d') > -1">
          <el-form ref="form"
                   :model="pfForm"
                   :label-width="pfLabelWidth">
            <el-row :gutter="20">
              <el-col :md="12"
                      class="maxwidth">
                <el-form-item label="新开业绩时间 :">
                  <el-date-picker v-model="pfForm.newPFdate"
              ...
              <el-col :md="12"
                ...
                <el-form-item label="上线业绩时间 :">
                  <el-date-picker v-model="pfForm.onlinePFdate"
                                  value-format="yyyy/MM/dd HH:mm"
                                  type="date"
                                  style="width:100%"></el-date-picker>
                </el-form-item>
            ...
            <div>
              <el-button @click.native="addPFTime"
    ...
    <el-dialog title="修改订单基本信息"
      ...
      <el-form :model="form">
        ...
        <el-row :gutter="20"
                v-for="(c,index) in form.contactList"
                :key="index">
          <el-col :sm="12">
            <el-form-item :label-width="formLabelWidth"
                          required>
              <span slot="label">
                <el-button v-show="index===0" @click="addContact(index)"
                           type="success"
                           circle
                           size="mini"
                           :icon="addIcon"></el-button>
          ...
          <el-col :sm="12">
            <el-form-item :label-width="formLabelWidth"
              ...
              <el-button @click="addContact(index,'del')"
                           type="danger"
                           circle
                           size="mini"
                           :icon="minusIcon"></el-button>
            </el-form-item>
          </el-col>
          <el-col :sm="12">
            <el-form-item :label-width="formLabelWidth">
      ...
      <div slot="footer"
        ...
        <el-button type="primary"
                   @click="subBaseInfo">提 交</el-button>
      </div>
    </el-dialog>
    <!-- 编辑资质弹窗 -->
    <el-dialog title="编辑资质"
               width="800px"
               :visible.sync="editQualifyDialog"
               :modal-append-to-body="false">
      <show-qualify v-if="copy_showQualify.length"
      :showQualify="copy_showQualify" class="qualify-left"></show-qualify>
      <div>
        <el-select  value-key="id" @change="qualifySelectChange" v-model="form.zizhiName" placeholder="请选择上传资质类型" class="input-btn">
...
export default {
  ...
  methods: {
    ...
    addQualify () {
      ...
      if (this.copy_showQualify.some(val => val.label === this.selectedQualify.tb_field_name)) {
