...
import { getByCode, getMyContract } from 'api/getOptions' //eslint-disable-line
  ...
  methods: {
    ...
    subOrder (subType) {
      ...
      if (!params.companyid || !params.receiptid || !params.companyaddress ||
          !params.websitetype || !params.custom_type || !params.invoice_status) {
        this.$message({
          type: 'error',
          message: '请完善带*号必填项'
        })
      ...
      this.$post('/wf.do?go', params).then(res => {
        if (res.data[0].success && !this.showEditBD) {
          this.$router.push({
